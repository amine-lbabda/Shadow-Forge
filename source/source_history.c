#include "history.h"
void history_menu(SDL_Surface *ecran, int *run, TTF_Font *font, SDL_Color coleur_text)
{
    SDL_Rect positionhist, positionreturn;
    SDL_Surface *menu_hist = NULL;
    SDL_Surface *return_hist = NULL;
    int quit = 0;
    return_hist = TTF_RenderText_Blended(font, "RETURN", coleur_text);
    ...
    while (quit == 0 && *run)
    {
        ...
        SDL_BlitSurface(menu_hist, NULL, ecran, &positionreturn);
