...
int main(int argc, char **argv)
{
    ...
    while (quitter)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            case SDL_KEYDOWN:
                ...
                else if (event.key.keysym.sym == SDLK_SPACE)
                {
                    ...
                    {
                        if (joueurs[0].double_saut_dispo == 1 && joueurs[0].vitesse_y > -300)
                        {
                            joueurs[0].vitesse_y = -380;
        ...
        printf("x logique = %d | x affich̩ = %d\n", joueurs[0].pos.x, joueurs[0].posscreen.x);
        ...
        if (nb_joueurs == 2)
        {
            ...
            SDL_Rect cam1 = {joueurs[0].pos.x + joueurs[0].pos.w / 2 - viewport1.w / 2,
                             joueurs[0].pos.y + joueurs[0].pos.h / 2 - viewport1.h / 2, viewport1.w, viewport1.h};

            SDL_Rect cam2 = {joueurs[1].pos.x + joueurs[1].pos.w / 2 - viewport2.w / 2,
                             joueurs[1].pos.y + joueurs[1].pos.h / 2 - viewport2.h / 2, viewport2.w, viewport2.h};
        ...
        else
        {
            ...
            else
            {
                ...
                SDL_Rect src1 = {scroll_start, bg.poscamera.y, largeur_restante, bg.poscamera.h};
                SDL_Rect dest1 = {0, 0};
                SDL_BlitSurface(bg.image, &src1, ecran, &dest1);

                SDL_Rect src2 = {0, bg.poscamera.y, scroll_width - largeur_restante, bg.poscamera.h};
            ...
            if (pack_visible)
            {
                SDL_Rect health_pack_screen = {health_pack.x - camX, health_pack.y - camY, health_pack.w,
                                               health_pack.h};
